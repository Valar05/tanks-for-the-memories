...
const expectedBuild = 'tftm-authored-sherman-treads-v1-10-20260705';
const expectedRevision = 'v1-10-restored-continuous-belt-shading';
const previousRedVerdictPath = 'docs/visual-verdicts/treads-v1-9-red.json';
const failures = [];
...
if (failures.length === 0) {
  ...
  if (previousRedVerdict.revision !== 'v1-9-open-running-gear-rebuild' || previousRedVerdict.status !== 'red_unaccepted_wrong_geometry_rebuild') {
    fail('previous v1.9 wrong-geometry rebuild verdict must remain explicit before v1.10 shading-only review');
  }
...
console.log('Tread-first cloud review gate passed: hosted packet declares v1.10 restored continuous belt shading review lane with preserved camera controls; cloud/Sense acceptance is still required.');
