...
const expectedBuild = 'tftm-authored-sherman-treads-v1-8c-20260705';
const expectedRevision = 'v1-8c-linked-mirror-tread-assembly';
...
if (failures.length === 0) {
  ...
  if (!review) fail('cloud manifest must include authored_treads_review');
    ...
    for (const phrase of ['full tread assembly only', 'open perimeter sidewall frame', 'wheels inside the inner profile opening', 'sprockets, idlers, return rollers, bogie connectors', 'profile opening', 'baked wheel rim loops', 'smooth rounded rubber faces', 'left/right tread nodes share mesh data as mirrored instances', 'preserve OrbitControls camera and orientation widget', 'no hull, turret, barrel, coaxial MG, or full tank scene', 'local capture was not used']) {
...
console.log('Tread-first cloud review gate passed: hosted packet declares v1.8c linked mirrored tread assembly review lane with preserved camera controls; cloud/Sense acceptance is still required.');
