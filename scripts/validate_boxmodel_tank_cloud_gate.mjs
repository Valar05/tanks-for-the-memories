...
if (failures.length === 0) {
  ...
  if (!rules.includes('tftm-authored-sherman-boxmodel-v1-13-20260705')) fail('cloud manifest must require the boxmodel build token');
  ...
  if (!rules.includes('recessed under-sponson shadow backing')) fail('cloud manifest must require recessed under-sponson shadow backing review');
  if (!rules.includes('visible horizontal shadow slot')) fail('cloud manifest must require visible horizontal shadow-slot review');
  if (!rules.includes('without floating boxes or runtime overlays')) fail('cloud manifest must reject floating boxes/runtime overlays as the coverage fix');
  ...
  if (!rules.includes('recessed backing panels')) fail('cloud manifest must require recessed backing panels, not rejected plug geometry');
  ...
  if (!captures.includes('front-right/front-left/rear-right/rear-left')) fail('cloud captures must require four named flat panel review');
