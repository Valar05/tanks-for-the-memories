...
if (failures.length === 0) {
  ...
  if (!rules.includes('tftm-authored-sherman-boxmodel-v1-9-20260704')) fail('cloud manifest must require the boxmodel build token');
  ...
  if (!rules.includes('exterior front gap coverage')) fail('cloud manifest must require exterior front gap coverage review');
  if (!rules.includes('visible outside side plane')) fail('cloud manifest must require visible outside side-plane review');
  ...
  if (!rules.includes('flat armor panels')) fail('cloud manifest must require flat armor panels, not rejected plug geometry');
