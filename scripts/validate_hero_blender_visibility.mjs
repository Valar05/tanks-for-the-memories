...
const root = 'generated/blender-visibility/authored_sherman_hero_v1';
...
if (!existsSync(manifestPath)) fail('missing Blender visibility manifest; run npm run blender-visibility:hero');
let manifest = null;
...
if (manifest) {
  ...
  if (!String(manifest.source_blend || '').endsWith('authored_sherman_hero_v1.blend')) fail('manifest must name source blend');
