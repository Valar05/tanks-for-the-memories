...
const releaseManifest = {
  ...
  authored_boxmodel_review: {
    ...
    tuner_expected_build: 'tftm-authored-sherman-boxmodel-tuner-v1-20260704',
    ...
    tuner_acceptance: 'Sense Simulation must review boxmodel-tank.html?tune=1 as a gesture-only boxmodel part tuner: parts list visible, one selected editable part highlighted, move/rotate/scale gestures or numeric edits visibly change that part, right-side camera rotation still works, no 3D gizmo appears, and local capture was not used.'
  },
  ...
  required_cloud_captures: [
    ...
    'boxmodel-tank.html?tune=1 phone portrait showing gesture-only part tuner, selected part highlight, parts list, and build token tftm-authored-sherman-boxmodel-tuner-v1-20260704',
    'boxmodel-tank.html?tune=1 cloud interaction evidence showing one selected part visibly move/rotate/scale through gestures or numeric controls, right-side camera rotation preserved, and no 3D gizmo',
  ...
  sense_simulation_questions: [
    ...
    'On boxmodel-tank.html?tune=1, does the gesture-only parts workflow let one selected armor/filler part be edited at a time without a 3D gizmo?',
