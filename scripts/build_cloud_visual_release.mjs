...
const releaseManifest = {
  ...
  authored_boxmodel_review: {
    ...
    tuner_expected_build: 'tftm-authored-sherman-boxmodel-tuner-v2-20260704',
    ...
    tuner_acceptance: 'Sense Simulation must review boxmodel-tank.html?tune=1 as a gesture-only boxmodel part tuner: parts list visible, four hull-colored plug instances are available for front/back/left/right holes, one selected plug is highlighted for editing, already enabled plugs remain visible, move/rotate/scale gestures or numeric edits visibly change the selected plug, right-side camera rotation still works, no 3D gizmo appears, and local capture was not used.'
  },
  ...
  required_cloud_captures: [
    ...
    'boxmodel-tank.html?tune=1 phone portrait showing gesture-only part tuner with front/back/left/right hull-colored plug parts, selected plug highlight, parts list, and build token tftm-authored-sherman-boxmodel-tuner-v2-20260704',
  ...
  sense_simulation_questions: [
    ...
    'On boxmodel-tank.html?tune=1, does the gesture-only parts workflow provide four front/back/left/right hull-colored plugs while still editing only one selected plug at a time without a 3D gizmo?',
