...
REVISION = 'v1-10-restored-continuous-belt-shading'
...
manifest = {
    ...
    'component_scope': 'full tread assembly only: restored continuous tread belt geometry, wheels inside the inner profile opening, sprockets, idlers, return rollers, bogie connectors, and connector mounts; no hull, turret, barrel, coaxial MG, full tank scene, or texture variant',
    'shading_contract': 'old continuous tread belt geometry is preserved; tread belt uses exported custom loop normals from profile tangents so shading, not geometry redesign or edge-lined panels, removes faceting',
    ...
    'acceptance': 'Cloud/Sense must judge treadfirst-treads.html only: restored continuous tread belt geometry with wheels, sprockets, idlers, return rollers, and bogie arms visibly occupying the inner tread profile opening; exported GLB normals must prove hard wheel rim-loop splits, smooth rounded rubber faces, and custom profile-tangent tread belt normals without faceted or edge-lined tread panels; no hull/turret/full-tank salvage.'
}
