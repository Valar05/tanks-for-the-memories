...
import { AUTHORED_SHERMAN_RETOPO_GLB_URL } from './sherman-asset-links';
import { applyAuthoredRetopoTexturePlates } from './sherman-runtime-materials';
...
root.innerHTML = '<main class="single-tank-shell">' +
  ...
  '<section class="single-tank-readout" aria-label="Authored Sherman retopo readout">' +
    '<p class="single-tank-kicker">authored retopo asset</p>' +
    '<p class="single-tank-title">Sherman face-plate review</p>' +
    '<p class="single-tank-status" data-status>loading authored Sherman retopo</p>' +
  '</section>' +
...
new GLTFLoader().load(AUTHORED_SHERMAN_RETOPO_GLB_URL, (gltf) => {
  const model = gltf.scene;
  model.name = 'authored_sherman_retopo_v1';
  applyAuthoredRetopoTexturePlates(model);
  ...
  tankRoot.add(model);
  statusEl.textContent = 'loaded authored retopo with split face plates';
  postVisualBeacon('loaded');
}, undefined, (error) => {
  statusEl.textContent = 'authored retopo load failed';
...
function postVisualBeacon(stage: string, extra: Record<string, string | number> = {}) {
  const params = new URLSearchParams({
    stage,
    build: visualBuild,
    actor: 'authored_sherman_retopo_v1',
    clip: 'split face texture plate review',
    clipKey: 'retopo-tank',
