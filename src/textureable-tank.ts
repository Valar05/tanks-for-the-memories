...
import { AUTHORED_SHERMAN_TEXTUREABLE_GLB_URL } from './sherman-asset-links';
import { applyAuthoredTextureableTexturePlates } from './sherman-runtime-materials';

const root = document.querySelector<HTMLDivElement>('#textureable-tank-root');
if (!root) throw new Error('missing #textureable-tank-root');

const visualBuild = 'tftm-authored-sherman-textureable-v1-1-20260705';

root.innerHTML = '<main class="single-tank-shell">' +
  '<div class="single-tank-stage"><canvas aria-label="Textureable Sherman base scene"></canvas></div>' +
  '<section class="single-tank-readout" aria-label="Authored Sherman retopo readout">' +
    '<p class="single-tank-kicker">authored textureable asset</p>' +
    '<p class="single-tank-title">Sherman texture base review</p>' +
    '<p class="single-tank-status" data-status>loading textureable Sherman base</p>' +
  '</section>' +
...
new GLTFLoader().load(AUTHORED_SHERMAN_TEXTUREABLE_GLB_URL, (gltf) => {
  const model = gltf.scene;
  model.name = 'authored_sherman_textureable_v1';
  applyAuthoredTextureableTexturePlates(model);
  ...
  tankRoot.add(model);
  statusEl.textContent = 'loaded textureable Sherman with contained track pods and split UV plates';
  postVisualBeacon('loaded');
}, undefined, (error) => {
  statusEl.textContent = 'textureable Sherman load failed';
...
function postVisualBeacon(stage: string, extra: Record<string, string | number> = {}) {
  const params = new URLSearchParams({
    stage,
    build: visualBuild,
    actor: 'authored_sherman_textureable_v1',
    clip: 'textureable tank review',
    clipKey: 'textureable-tank',
