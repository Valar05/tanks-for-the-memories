...
### Authored Textureable Base - 2026-07-05

The active recovery path after the red boxmodel screenshots is `authored_sherman_textureable_v1`, generated by `scripts/export_authored_sherman_textureable.py`. Treat `authored_sherman_boxmodel_v1` as red evidence unless cloud/Sense later accepts it; do not keep polishing it as the texture base by default.
...
The next clean recovery step is `authored_sherman_treads_v1`, reviewed through `treadfirst-treads.html`. This is a component pass, not a full tank pass: it must contain the full tread assembly only: `treads_root`, `left_tread_belt`, `right_tread_belt`, `left_tread_connector_mounts`, `right_tread_connector_mounts`, `left_wheel_group`, `right_wheel_group`, `left_bogie_connectors`, and `right_bogie_connectors` plus subordinate road wheels, sprockets, idlers, return rollers, bogie arms, and connector meshes. Hull, turret, barrel, mantlet, coaxial MG, and texture-variant work are forbidden in this pass.

Use `src/model-assay.ts` `createTreadGeometry()` only as the subdivision-0 reference for tread proportions. The authored tread asset must add one silhouette subdivision layer around the front/rear returns and top/bottom transitions, while preserving a closed Sherman-like trapezoid belt volume with visible top, bottom, front, rear, inner, and outer thickness and side-facing running gear inside that belt. Failed `authored_sherman_boxmodel_v1` and `authored_sherman_textureable_v1` exporters are red evidence only and must not be copied as architecture for this pass.
...
The v1.4 tread refinement preserves the v1.3 wheel/profile placement and changes only shading/readability: tread belt plates keep hard flat normals and bevel corners, while wheel geometry separates flat metal rim faces from smooth rubber tire sidewalls. Do not reintroduce radial tire facets on the rubber ring to create fake detail; use rim lips, hard edge normals, material separation, and Sense review instead.
