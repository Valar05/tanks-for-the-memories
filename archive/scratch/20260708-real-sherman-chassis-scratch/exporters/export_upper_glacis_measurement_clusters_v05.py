...
ASSET_ID = 'real_sherman_upper_glacis_cluster_owned_silhouette_scratch_v05'
REVISION = 'v05-cluster-owned-silhouette-bounded-closed-solids'
...
def feature_from_cluster(name, cluster, fallback_plane, thickness, source_note):
    ...
    # v05 uses cluster centroids only for authored boundaries. Face vertices and
...
def discover_feature_clusters(clusters, source_uv_bounds):
    ...
    # Unique ownership is the main v05 rule. Side returns must be side-normal
...
def add_uvs(obj, name='uv_cluster_owned_v05'):
    mesh = obj.data
...
def write_reports(features, output_objects, topology_reports, topology_gate, depth_report, area_report, primitive_rows):
    ...
    manifest = {
        ...
        'current_prompt_contract': {
            'current_user_command': 'Implement v05 silhouette-improvement scratch pass from PR review.',
            ...
            'intended_mutation': 'Create a new v05 exporter and artifacts that enforce unique feature clusters, side-normal returns, and silhouette-bounded closed solids.',
