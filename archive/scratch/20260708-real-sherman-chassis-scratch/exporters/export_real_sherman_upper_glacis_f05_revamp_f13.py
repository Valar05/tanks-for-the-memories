"""Scratch exporter for the cleaner no-hatch-cap watertight socket pass.

Assumptions:
- Hatch cap modeling is deliberately omitted; this pass tests only solid plate, socket, and ring.
- Broad plate vertices are still source-boundary driven, so this exporter is expected to be diagnostic rather than final.
- Candidate language is blocked unless shape diagnostics stop looking like the prior slab failures.
"""
import json
import math
...
ASSET_ID = 'real_sherman_upper_glacis_f05_revamp_scratch_f13'
REVISION = 'f13-watertight-clean-cylinder-ring-no-hatch-cap'
...
def measure_source_features(source_obj):
    ...
    if min_outer_dist < socket_radius + RING_RADIAL_WIDTH + 0.035:
        ring_center = center
    ring_base_z = z_at(bvh, ring_center.x + socket_radius, ring_center.y, q(z_vals, 0.78)) + 0.002
    # Low-pass the source-supported outer loop so broad armor remains manufactured, not jagged scanned noise.
    smoothed_outer = []
    for i, p in enumerate(outer):
        acc = Vector((0, 0, 0))
        weight = 0.0
        for off, w in [(-2, 0.10), (-1, 0.22), (0, 0.36), (1, 0.22), (2, 0.10)]:
            qpt = outer[(i + off) % len(outer)]
            acc += qpt * w
            weight += w
        sp = acc / weight
        sp.z = z_at(bvh, sp.x, sp.y, p.z)
        smoothed_outer.append(sp)
    outer = smoothed_outer
    return {
        'verts': verts,
        'polys': polys,
        'bvh': bvh,
...
def make_hatch_cap(features, mats):
    ...
    for p in mesh.polygons:
        p.material_index = 3
        p.use_smooth = False
    obj = bpy.data.objects.new(ASSET_ID + '_separate_omitted_hatch_cap', mesh)
    bpy.context.collection.objects.link(obj)
    obj['asset_id'] = ASSET_ID
    obj['role'] = 'separate_omitted_hatch_cap'
...
def main():
    ...
    hatch_primitive = {'omitted': True, 'reason': 'new correction forbids continuing hatch-cap modeling in this pass'}
    ...
    manifest = {
        'asset_id': ASSET_ID,
        'scratch_mode': True,
        'artifact_type': 'scratch_watertight_upper_glacis_solid_plate_socket_ring_no_hatch_cap',
        'revision': REVISION,
        'candidate': False,
        ...
        'authoring_policy': 'New closed hard-surface solids measured from source/f05 reference. f05/f11 topology is not used as final mesh base. Top plate is solid, aperture has cylindrical wall ownership, raised ring is a separate closed mesh, and hatch cap geometry is omitted per the latest correction.',
        'current_prompt_contract': {
            ...
            'intended_mutation': 'Create one new scratch mesh artifact with watertight solid plate, cylindrical cut, separate closed ring, no hatch cap, objective topology gate, notes, and viewer handoff.',
