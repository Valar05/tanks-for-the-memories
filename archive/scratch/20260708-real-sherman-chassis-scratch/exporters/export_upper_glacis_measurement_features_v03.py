"""Feature-decomposition hard-surface reconstruction for the Sherman upper glacis.

This scratch exporter is the reviewed successor to measurement-solids v01/v02.
It must not tune v02 constants into a prettier slab. The source mesh remains a
measuring instrument only: it supplies named feature planes, bounds, seams, and
socket/ring measurements. The exported geometry is fresh authored closed solids.
...
ASSET_ID = 'real_sherman_upper_glacis_measurement_features_scratch_v03'
REVISION = 'v03-named-feature-measurements-authored-closed-solids'
...
def feature_from_bounds(name, global_rows, fallback_plane, uv_bounds, thickness):
    rows = [r for r in global_rows if uv_bounds['u_min'] <= r['u'] <= uv_bounds['u_max'] and uv_bounds['v_min'] <= r['v'] <= uv_bounds['v_max']]
    plane = make_feature_plane(name, rows, fallback_plane)
    corners = corners_from_uv_bounds(plane, uv_bounds)
    return {
        'name': name,
        'plane': plane,
        'uv_bounds': uv_bounds,
...
def measure_features(source_obj):
    ...
    for fi, face in enumerate(polys):
        ...
        face_rows.append({'face_index': fi, 'centroid': centroid, 'normal': normals[fi], 'area': area, 'u': u, 'v': v})
    z_vals = [p.z for p in verts]
    ...
    bounds = {
        'main_glacis': {
            'u_min': u_min + u_span * 0.18,
            'u_max': u_max - u_span * 0.18,
            'v_min': v_min + v_span * 0.24,
            'v_max': v_max - v_span * 0.24,
        },
        'left_shoulder': {
            'u_min': u_min,
            'u_max': u_min + u_span * 0.28,
            'v_min': v_min + v_span * 0.14,
            'v_max': v_max - v_span * 0.06,
        },
        'right_shoulder': {
            'u_min': u_max - u_span * 0.28,
            'u_max': u_max,
            'v_min': v_min + v_span * 0.14,
            'v_max': v_max - v_span * 0.06,
        },
        'front_face': {
            'u_min': u_min + u_span * 0.04,
            'u_max': u_max - u_span * 0.04,
            'v_min': v_min,
            'v_max': v_min + v_span * 0.30,
        },
        'deck_transition': {
            'u_min': u_min + u_span * 0.08,
            'u_max': u_max - u_span * 0.08,
            'v_min': v_max - v_span * 0.30,
            'v_max': v_max,
        },
        'left_return': {
            'u_min': u_min,
            'u_max': u_min + u_span * 0.16,
            'v_min': v_min,
            'v_max': v_max,
        },
        'right_return': {
            'u_min': u_max - u_span * 0.16,
            'u_max': u_max,
            'v_min': v_min,
            'v_max': v_max,
        },
    ...
    for name in FEATURE_ORDER:
        features[name] = feature_from_bounds(name, face_rows, fallback_plane, bounds[name], THICKNESS_BY_FEATURE[name])
    # Measure socket/ring from the same cloud but do not let them define the hull plates.
    ...
    socket_plane = features['main_glacis']['plane']
    features['socket'] = {
        'name': 'socket',
        'plane': socket_plane,
        'uv_bounds': {
            'u_min': ring_u - socket_radius,
            'u_max': ring_u + socket_radius,
            'v_min': ring_v - socket_radius,
            'v_max': ring_v + socket_radius,
        },
        'world_corners': [],
        'thickness': SOCKET_DEPTH,
        'source_sample_count': len(ring_candidates),
        'status': 'pass' if len(ring_candidates) >= MIN_REQUIRED_SAMPLES else 'red_under_sampled',
        'center_uv': [ring_u, ring_v],
        'center_world': uv_to_plane(socket_plane, ring_u, ring_v, 0.0),
    ...
    features['ring'] = {
        ...
        'plane': socket_plane,
        'uv_bounds': {
            'u_min': ring_u - socket_radius - RING_RADIAL_WIDTH,
            'u_max': ring_u + socket_radius + RING_RADIAL_WIDTH,
            'v_min': ring_v - socket_radius - RING_RADIAL_WIDTH,
            'v_max': ring_v + socket_radius + RING_RADIAL_WIDTH,
        },
        'world_corners': [],
        ...
        'center_uv': [ring_u, ring_v],
        'center_world': uv_to_plane(socket_plane, ring_u, ring_v, 0.0),
        'inner_radius': socket_radius + 0.004,
        'outer_radius': socket_radius + 0.004 + RING_RADIAL_WIDTH,
    }
    ...
    return {
        ...
        'source_uv_bounds': {'u_min': u_min, 'u_max': u_max, 'v_min': v_min, 'v_max': v_max},
    }


def add_uvs(obj, name='uv_measurement_features_v03'):
    mesh = obj.data
...
def make_closed_plate_from_feature(feature, name, mat, role):
    ...
    return make_mesh_object(
        ...
        mat,
        role,
        'closed plate prism authored from named feature world_corners and feature plane normal',
    )
...
def write_missing_feature_manifest(features):
    manifest = {
        'asset_id': ASSET_ID,
        'scratch_mode': True,
        'artifact_type': 'measurement_feature_decomposition_upper_glacis',
        'revision': REVISION,
        'candidate': False,
        'shape_review_status': 'red_missing_required_feature',
    ...
    (MODEL_DIR / 'measurement_report.json').write_text(json.dumps({
        'asset_id': ASSET_ID,
        'status': 'red_missing_required_feature',
...
def write_reports(features, output_objects, topology_reports, topology_gate, depth_report, area_report, primitive_rows):
    ...
    measurement_path.write_text(json.dumps({
        'asset_id': ASSET_ID,
        'status': 'pass' if not features['missing_required_features'] else 'red_missing_required_feature',
    ...
    shape_status = 'diagnostic_promising' if depth_report['silhouette_iou'] > 0.62 and area_report['status'] == 'pass' else 'red_depth_or_silhouette'
    ...
    manifest = {
        ...
        'current_prompt_contract': {
            'current_user_command': 'Implement v03 feature-decomposition scratch pass from PR review.',
            ...
            'intended_mutation': 'Create a new v03 exporter and artifacts that measure named manufactured hull features and author separate closed solids.',
...
def main():
    ...
    primitive_rows = []
    for name in FEATURE_ORDER:
        obj = make_closed_plate_from_feature(features['features'][name], ASSET_ID + '_' + name + '_closed_solid', mats[name], name)
        objects.append(obj)
