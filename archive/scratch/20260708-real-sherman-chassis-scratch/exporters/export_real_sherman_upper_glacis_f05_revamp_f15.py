"""Incomplete scratch exporter for recovering f13-style boundary on the f14 fitted plane.

Assumptions:
- Use the broader source boundary for silhouette, then project that loop to a fitted plane.
- Do not return to raycast heights for broad armor.
- This attempt is preserved because it hit a headless render/export problem before producing a GLB.
"""
import json
import math
...
ASSET_ID = 'real_sherman_upper_glacis_f05_revamp_scratch_f15'
REVISION = 'f15-planar-fit-f13-boundary-recovery'
...
def measure_source_features(source_obj):
    ...
    if len(top_points) < 20:
        top_points = verts
    world_center = mean(top_points)
    deck_candidates = [(p, *plane_to_uv(plane, p)) for p in top_points]
    center_u, center_v = plane_to_uv(plane, world_center)
    ring_points = []
    for p in top_points:
        r = math.sqrt((p.x - world_center.x) ** 2 + (p.y - world_center.y) ** 2)
        if 0.09 <= r <= 0.36 and p.z >= q(z_vals, 0.72):
            ring_points.append(p)
    if len(ring_points) < 30:
        ring_points = top_points
    ring_center_world = mean(ring_points)
    ring_u, ring_v = plane_to_uv(plane, ring_center_world)
    ring_radii = sorted(math.hypot(u - ring_u, v - ring_v) for _p, u, v in [(p, *plane_to_uv(plane, p)) for p in ring_points])
    ...
    for i in range(SEGMENTS):
        ...
        candidates = []
        for p in top_points:
            d = angle_distance(math.atan2(p.y - world_center.y, p.x - world_center.x), a)
            if d <= math.tau / SEGMENTS * 2.4:
                r = math.sqrt((p.x - world_center.x) ** 2 + (p.y - world_center.y) ** 2)
                u, v = plane_to_uv(plane, p)
                candidates.append((r, u, v))
        if not candidates:
            p = max(top_points, key=lambda pp: math.sqrt((pp.x - world_center.x) ** 2 + (pp.y - world_center.y) ** 2) - angle_distance(math.atan2(pp.y - world_center.y, pp.x - world_center.x), a) * 0.18)
            r = math.sqrt((p.x - world_center.x) ** 2 + (p.y - world_center.y) ** 2)
            u, v = plane_to_uv(plane, p)
        else:
...
def write_plane_and_landmark_reports(features, output_objects, plane_gate):
    ...
    landmarks = {
        'asset_id': ASSET_ID,
        'loops': {
            'outer_silhouette_loop': {'kind': 'f13_style_world_xy_boundary_projected_to_fitted_plane_uv', 'samples': len(features['outer_loop_uv'])},
...
def main():
    ...
    primitive_path.write_text(json.dumps({
        ...
        'ring': ring_primitive,
        'hatch': {'omitted': True, 'reason': 'do not continue hatch-cap modeling in this pass'},
        'source_feature_policy': 'source samples solve broad boundary and landmarks only; f13-style boundary points are projected to fitted deck plane, not raycast back onto source noise.',
    }, indent=2) + '\n')
    ...
    manifest = {
        'asset_id': ASSET_ID,
        'scratch_mode': True,
        'artifact_type': 'scratch_planar_fit_watertight_upper_glacis_f13_boundary_recovery',
        'revision': REVISION,
        'candidate': candidate,
        ...
        'authoring_policy': 'Dominant deck plane is fitted from source faces. Broad armor vertices use source-supported f13-style boundary selection in world XY, then are projected to fitted plane; source samples are not used for broad-plate height.',
        'current_prompt_contract': {
            'current_user_command': 'Implement planar-fit watertight pass and recover broad source boundary if f14 shape collapses.',
            ...
            'intended_mutation': 'Create one new scratch mesh artifact with fitted planar armor, f13-style broad source boundary projected to plane, cylindrical cut, separate closed ring, no hatch cap, plane/topology/shape diagnostics, and viewer handoff.',
    ...
    note += '## Process Fix\n\nBroad armor is generated from a fitted deck plane. The outer loop recovers the broader f13-style source boundary in world XY, then projects it to the plane; broad top-plate vertices are not raycast back onto source height noise.\n\n'
    note += '## Gates\n\n'
