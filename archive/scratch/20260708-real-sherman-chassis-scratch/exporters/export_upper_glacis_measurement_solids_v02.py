...
ASSET_ID = 'real_sherman_upper_glacis_measurement_solids_scratch_v02'
REVISION = 'v02-measured-lower-armor-volume-authored-solids'
...
LOWER_APRON_DROP_MIN = 0.14
LOWER_APRON_DROP_MAX = 0.30
LOWER_APRON_THICKNESS = 0.060
...
def make_lower_apron_solid(features, mat):
    plane = features['plane']
    left0 = features['stations']['left'][0]
    right0 = features['stations']['right'][0]
    zq = features['source_z_quantiles']
    measured_drop = max(LOWER_APRON_DROP_MIN, min(LOWER_APRON_DROP_MAX, zq['z70'] - zq['z08']))
    cu, cv = features['ring_center_uv']
    a = Vector((right0[0] - left0[0], right0[1] - left0[1], 0))
    if a.length < 1e-8:
        a = Vector((1, 0, 0))
    a.normalize()
    outward = Vector((-a.y, a.x, 0))
    mid = ((left0[0] + right0[0]) * 0.5, (left0[1] + right0[1]) * 0.5)
    if Vector((mid[0] - cu, mid[1] - cv, 0)).dot(outward) < 0:
        outward = -outward
    # This is an authored armor wall: the source measures the edge, drop, and outward side;
    # no source face or vertex is reused as output topology.
    top_front = [left0, right0]
    top_back = [(u - outward.x * LOWER_APRON_THICKNESS, v - outward.y * LOWER_APRON_THICKNESS) for u, v in top_front]
    low_front = top_front
    low_back = top_back
    verts = [
        uv_to_plane(plane, top_front[0][0], top_front[0][1], -0.006),
        uv_to_plane(plane, top_front[1][0], top_front[1][1], -0.006),
        uv_to_plane(plane, top_back[1][0], top_back[1][1], -0.014),
        uv_to_plane(plane, top_back[0][0], top_back[0][1], -0.014),
        uv_to_plane(plane, low_front[0][0], low_front[0][1], -measured_drop),
        uv_to_plane(plane, low_front[1][0], low_front[1][1], -measured_drop),
        uv_to_plane(plane, low_back[1][0], low_back[1][1], -measured_drop),
        uv_to_plane(plane, low_back[0][0], low_back[0][1], -measured_drop),
    ]
    faces = [
        (0, 1, 2, 3),  # upper seam cap
        (4, 7, 6, 5),  # lower closure
        (0, 4, 5, 1),  # visible lower armor face
        (1, 5, 6, 2),  # right end cap
        (2, 6, 7, 3),  # back face
        (3, 7, 4, 0),  # left end cap
    ]
    obj = make_mesh_object(ASSET_ID + '_lower_hull_apron_closed_solid', verts, faces, mat, 'lower_hull_seam', 'closed lower armor/apron solid measured from lower seam and source vertical depth')
    obj['measured_drop'] = measured_drop
    return obj, {'region': 'lower_hull_seam', 'topology': 'closed lower apron armor solid', 'source_use': 'measured lower seam endpoints and source z-depth quantiles only', 'measured_drop': measured_drop, 'thickness': LOWER_APRON_THICKNESS}
...
def make_returns_and_seam(features, mat_return, mat_seam):
    ...
    seam_obj, seam_row = make_lower_apron_solid(features, mat_seam)
