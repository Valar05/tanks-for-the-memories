...
This scratch exporter is the reviewed successor to v06. It removes fixed UV
...
REVISION = 'v06-plane-locked-primary-armor-clusters'
...
def feature_from_cluster(name, cluster, fallback_plane, thickness, source_note):
    ...
    # v06 uses cluster centroids only for authored boundaries. Face vertices and
    # merged debris made v04 expand into shelf slabs even when cluster identity was useful.
    points = [(r['u'], r['v']) for r in cluster['members']]
    hull = simplify_polygon(convex_hull_2d(points), 8)
...
def write_reports(features, output_objects, topology_reports, topology_gate, depth_report, area_report, primitive_rows):
    ...
    manifest = {
        ...
        'current_prompt_contract': {
            'current_user_command': 'Implement v06 plane-locked silhouette-improvement scratch pass from PR review.',
            ...
            'intended_mutation': 'Create a new v06 exporter and artifacts that enforce unique feature clusters, side-normal returns, and one shared manufactured primary armor plane.',
    ...
    note += '## Construction Strategy\n\nCluster ownership plus primary plane locking replace the v05 tiered shelf planes. The source cloud is clustered by adjacency and face normals, then named manufactured regions are authored as fresh closed solids from cluster boundaries.\n\n'
    note += '## Gates\n\n'
