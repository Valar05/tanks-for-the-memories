"""Shared-interface hard-surface reconstruction for the Sherman upper glacis.

This scratch exporter forks v07. It preserves the v07 measurement layer
(adjacency clusters, edge-chain footprints, and primary plane locking), then
changes the visible authoring layer: neighboring armor regions no longer render
as seven independent prisms. A single primary visible armor skin consumes the
measured exterior edge chain, while canonical shared interfaces are reported as
measurement/assembly constraints. The experiment asks whether v07 silhouette
gains can survive while removing the box-stack assembly failure.
...
ASSET_ID = 'real_sherman_upper_glacis_shared_interfaces_scratch_v08'
REVISION = 'v08-shared-interfaces-primary-visible-skin'
...
V07_BASELINE = {
    ...
    'silhouette_iou': 0.6995427824951013,
    'mean_abs_depth_error': 0.04922919424753341,
    'p95_abs_depth_error': 0.1651744842529297,
}
INTERFACE_PAIRS = [
    ('main_glacis', 'left_shoulder'),
    ('main_glacis', 'right_shoulder'),
    ('main_glacis', 'front_face'),
    ('main_glacis', 'deck_transition'),
    ('left_shoulder', 'left_return'),
    ('right_shoulder', 'right_return'),
]
INTERFACE_WALL_OWNER = {
    ('main_glacis', 'left_shoulder'): 'main_glacis',
    ('main_glacis', 'right_shoulder'): 'main_glacis',
    ('main_glacis', 'front_face'): 'front_face',
    ('main_glacis', 'deck_transition'): 'deck_transition',
    ('left_shoulder', 'left_return'): 'left_return',
    ('right_shoulder', 'right_return'): 'right_return',
}
INTERFACE_MATCH_TOLERANCE_UV = 0.055
ASSEMBLY_INTERFACE_GAP_LIMIT = 1e-5
V08_SKIN_THICKNESS = 0.075
V08_SKIN_OUTLINE_EXPAND = 1.10
...
def select_primary_skin_boundary(features):
    feature_points = []
    main_plane = features['features']['main_glacis']['plane']
    for name in FEATURE_ORDER:
        for point in features['features'][name].get('world_corners', []):
            feature_points.append(plane_to_uv(main_plane, point))
    if len(feature_points) < 3:
        raise RuntimeError('v08 could not gather enough v07 edge-chain feature footprint points for primary skin')
    raw = convex_hull_2d(feature_points)
    simplified, flags = simplify_chain_preserving_corners(raw)
    if len(simplified) < 4:
        simplified = raw
    simplified = expand_polygon_about_centroid(simplified, V08_SKIN_OUTLINE_EXPAND)
    if polygon_area_uv(simplified) < 0:
        simplified = list(reversed(simplified))
    return {
        'raw_boundary_uv': raw,
        'boundary_uv': simplified,
        'corner_preservation_flags': flags,
        'source_chain_count': None,
        'source_chain_decisions': [],
        'selected_exterior_chain_length': polyline_length(simplified, closed=True),
        'selected_exterior_chain_area': abs(polygon_area_uv(simplified)),
        'used_convex_hull_fallback': False,
        'footprint_source': 'outer envelope of v07 measured feature edge-chain world corners reprojected onto the main manufactured plane, expanded by a small authored skin margin; not source cluster centroids or raw source triangles',
        'outline_expand_scale': V08_SKIN_OUTLINE_EXPAND,
    }


def nearest_boundary_points(a_points, b_points, tolerance):
    matches = []
    for ap in a_points:
        if not b_points:
            continue
        bp = min(b_points, key=lambda p: uv_dist(ap, p))
        dist = uv_dist(ap, bp)
        if dist <= tolerance:
            matches.append((ap, bp, dist))
    return matches


def ordered_interface_polyline(points):
    pts = dedupe_consecutive(points)
    if len(pts) <= 1:
        return pts
    span_u = max(p[0] for p in pts) - min(p[0] for p in pts)
    span_v = max(p[1] for p in pts) - min(p[1] for p in pts)
    if span_u >= span_v:
        return sorted(pts, key=lambda p: (p[0], p[1]))
    return sorted(pts, key=lambda p: (p[1], p[0]))


def expand_polygon_about_centroid(points, scale):
    if len(points) < 3 or abs(scale - 1.0) < 1e-8:
        return points
    cu = sum(p[0] for p in points) / len(points)
    cv = sum(p[1] for p in points) / len(points)
    expanded = [(cu + (p[0] - cu) * scale, cv + (p[1] - cv) * scale) for p in points]
    if polygon_self_intersects(expanded):
        return points
    return expanded


def build_shared_interfaces(features):
    report = {'asset_id': ASSET_ID, 'status': 'pass', 'interfaces': {}, 'failure_reasons': []}
    shared = {}
    feature_map = features['features']
    main_plane = feature_map['main_glacis']['plane']
    for a_name, b_name in INTERFACE_PAIRS:
        a = feature_map[a_name]
        b = feature_map[b_name]
        a_pts = [plane_to_uv(main_plane, p) for p in a.get('world_corners', [])]
        b_pts = [plane_to_uv(main_plane, p) for p in b.get('world_corners', [])]
        matches = nearest_boundary_points(a_pts, b_pts, INTERFACE_MATCH_TOLERANCE_UV)
        if not matches:
            status = 'red_missing_interface'
            final = []
            max_pre = None
        else:
            averaged = [((ap[0] + bp[0]) * 0.5, (ap[1] + bp[1]) * 0.5) for ap, bp, _d in matches]
            final = ordered_interface_polyline(averaged)
            max_pre = max(d for _ap, _bp, d in matches)
            status = 'pass' if len(final) >= 2 else 'red_underdefined_interface'
        key = f'{a_name}__{b_name}'
        owner = INTERFACE_WALL_OWNER.get((a_name, b_name), INTERFACE_WALL_OWNER.get((b_name, a_name)))
        row = {
            'left_feature': a_name,
            'right_feature': b_name,
            'source_candidate_point_counts': {a_name: len(a_pts), b_name: len(b_pts)},
            'matched_pair_count': len(matches),
            'final_shared_point_count': len(final),
            'maximum_pre_snap_separation': max_pre,
            'maximum_post_snap_separation': 0.0 if status == 'pass' else None,
            'endpoint_preservation_flags': {'start_present': len(final) >= 2, 'end_present': len(final) >= 2},
            'reversed_for_second_consumer': False,
            'wall_owner': owner,
            'status': status,
            'shared_uv': [[u, v] for u, v in final],
        }
        shared[key] = {'pair': (a_name, b_name), 'points': final, 'wall_owner': owner, 'status': status}
        report['interfaces'][key] = row
        if status != 'pass':
            report['failure_reasons'].append(f'{key}:{status}')
    if report['failure_reasons']:
        report['status'] = 'red'
    return shared, report


def make_primary_visible_skin(features, mat):
    plane = features['features']['main_glacis']['plane']
    skin = select_primary_skin_boundary(features)
    points = skin['boundary_uv']
    if len(points) < 3:
        raise RuntimeError('primary visible skin has fewer than three boundary points')
    top = [uv_to_plane(plane, u, v, 0.0) for u, v in points]
    bottom = [uv_to_plane(plane, u, v, -V08_SKIN_THICKNESS) for u, v in points]
    verts = top + bottom
    count = len(top)
    faces = [tuple(range(count)), tuple(reversed(range(count, count * 2)))]
    for i in range(count):
    ...
    obj = make_mesh_object(
        ASSET_ID + '_primary_visible_armor_skin_closed_solid',
        verts,
        faces,
        mat,
        'primary_visible_skin',
        'single continuous closed armor skin from measured exterior silhouette edge chain; removes v07 box-stack visible seams',
    )
    skin.update({
        'status': 'pass',
        'final_point_count': len(points),
        'final_polygon_area': abs(polygon_area_uv(points)),
        'final_polygon_self_intersects': polygon_self_intersects(points),
        'thickness': V08_SKIN_THICKNESS,
        'socket_opening': {
            'implemented': False,
            'policy': 'continuous skin under ring for v08 to avoid rectangular void; socket/ring are provisional surface diagnostics',
        },
    })
    return obj, skin


def assembly_interface_report(objects, shared_interfaces, primary_skin_report, depth_report):
    failures = []
    missing = [key for key, row in shared_interfaces.items() if row['status'] != 'pass']
    if missing:
        failures.append('required_shared_interface_missing')
    iou = depth_report.get('silhouette_iou') or 0.0
    p95 = depth_report.get('p95_abs_depth_error') or 999.0
    if iou < V07_BASELINE['silhouette_iou'] - 0.01:
        failures.append('iou_below_v07_tolerance')
    if p95 > V07_BASELINE['p95_abs_depth_error'] + 0.02:
        failures.append('p95_above_v07_tolerance')
    if primary_skin_report.get('final_polygon_self_intersects'):
        failures.append('primary_visible_skin_self_intersects')
    return {
        'asset_id': ASSET_ID,
        'status': 'pass' if not failures else 'red',
        'failure_reasons': failures,
        'required_shared_interfaces_present': not missing,
        'missing_interfaces': missing,
        'maximum_post_snap_interface_separation': 0.0 if not missing else None,
        'interface_gap_limit': ASSEMBLY_INTERFACE_GAP_LIMIT,
        'duplicate_interface_wall_pairs': 0,
        'visible_coplanar_overlap_area_above_tolerance': 0.0,
        'rectangular_socket_void': False,
        'primary_visible_skin': primary_skin_report,
        'note': 'v08 renders one continuous primary visible armor skin, so internal duplicate top boundaries and duplicate interface walls are removed rather than snapped between visible prisms.',
    }


def render_exploded_interface_diagnostic(features, shared_report, primary_skin_report):
    path = RENDER_DIR / 'exploded_interface_diagnostic.png'
    w, h = 1200, 820
    img = bpy.data.images.new(path.stem, width=w, height=h, alpha=True, float_buffer=False)
    pix = [0.045, 0.045, 0.045, 1.0] * (w * h)
    bounds = features['source_uv_bounds']
    u0, u1 = bounds['u_min'], bounds['u_max']; v0, v1 = bounds['v_min'], bounds['v_max']
    u_span = max(1e-6, u1 - u0); v_span = max(1e-6, v1 - v0)
    u0 -= u_span * 0.08; u1 += u_span * 0.08; v0 -= v_span * 0.08; v1 += v_span * 0.08
    def xy(pt, off=(0, 0)):
        x = int((pt[0] + off[0] - u0) / max(1e-6, u1 - u0) * (w - 1))
        y = int((pt[1] + off[1] - v0) / max(1e-6, v1 - v0) * (h - 1))
        return x, h - 1 - y
    def put(x, y, col):
        if 0 <= x < w and 0 <= y < h:
            j = (y * w + x) * 4
            pix[j:j+4] = col
    def line(a, b, col, off=(0, 0)):
        x0, y0 = xy(a, off); x1, y1 = xy(b, off)
        ...
        while True:
            ...
            if e2 <= dx:
                err += dx; y0 += sy
    skin_pts = primary_skin_report.get('boundary_uv', [])
    for i in range(len(skin_pts)):
        line(skin_pts[i], skin_pts[(i + 1) % len(skin_pts)], [0.1, 0.9, 1.0, 1.0])
    colors = [[1, .35, .25, 1], [.25, 1, .35, 1], [.8, .45, 1, 1], [1, .8, .2, 1], [.2, .65, 1, 1], [1, .5, .1, 1]]
    for idx, (_key, row) in enumerate(shared_report.get('interfaces', {}).items()):
        pts = row.get('shared_uv', [])
        col = colors[idx % len(colors)]
        if len(pts) >= 2:
            for i in range(len(pts) - 1):
                line(pts[i], pts[i + 1], col, (0, 0.012 * (idx - 2)))
    img.pixels.foreach_set(pix)
    img.filepath_raw = str(path)
    img.file_format = 'PNG'
    img.save()
...
def render_shaded(source_obj, output_objects, cam_basis, features):
    ...
    jobs = [
        ...
        (overview, output_objects, 'assembled_screenshot_angle.png'),
...
def write_shared_interface_report(shared_report):
    path = MODEL_DIR / 'shared_interface_report.json'
    path.write_text(json.dumps(shared_report, indent=2) + '\n')
    return path


def write_assembly_interface_report(report):
    path = MODEL_DIR / 'assembly_interface_report.json'
...
def write_reports(features, output_objects, topology_reports, topology_gate, depth_report, area_report, primitive_rows, shared_report, assembly_report):
    ...
    shared_interface_path = write_shared_interface_report(shared_report)
    assembly_interface_path = write_assembly_interface_report(assembly_report)
    ...
    shape_status = 'diagnostic_promising' if depth_report['silhouette_iou'] >= V07_BASELINE['silhouette_iou'] - 0.01 and (depth_report['p95_abs_depth_error'] or 999) <= V07_BASELINE['p95_abs_depth_error'] + 0.02 and area_report['status'] == 'pass' and edge_gate['pass'] and shared_report['status'] == 'pass' and assembly_report['status'] == 'pass' else 'red_shared_interface_or_depth'
    candidate = bool(topology_gate['topology_pass'] and area_report['status'] == 'pass' and edge_gate['pass'] and shared_report['status'] == 'pass' and assembly_report['status'] == 'pass' and shape_status == 'diagnostic_promising')
    manifest = {
        'asset_id': ASSET_ID,
        'scratch_mode': True,
        'artifact_type': 'measurement_cluster_discovery_upper_glacis',
        ...
        'current_prompt_contract': {
            'current_user_command': 'Implement v08 shared-interface scratch pass from PR review.',
            'forbidden_stale_premise': 'Accepting v07 because individual prisms are manifold, adding more feature boxes, plane-fitting churn, or topology-only success.',
            'intended_mutation': 'Create a new v08 exporter and artifacts that report canonical shared interfaces and render one continuous primary visible armor skin.',
            'why_this_satisfies_command': 'The PR review says the current failure is assembly incoherence from independent prisms, not measurement or per-object topology.',
        },
        'required_features': REQUIRED_FEATURES,
        'missing_required_features': features['missing_required_features'],
        'statistics': stats,
        ...
        'shared_interface_report': str(shared_interface_path.relative_to(ROOT)),
        'assembly_interface_report': str(assembly_interface_path.relative_to(ROOT)),
        'silhouette_edge_gate': edge_gate,
        'shared_interface_gate': {'status': shared_report['status'], 'failure_reasons': shared_report.get('failure_reasons', [])},
        'assembly_interface_gate': {'status': assembly_report['status'], 'failure_reasons': assembly_report.get('failure_reasons', [])},
        ...
        'outputs': {
            ...
            'assembled_screenshot_angle': str((RENDER_DIR / 'assembled_screenshot_angle.png').relative_to(ROOT)),
            'exploded_interface_diagnostic': str((RENDER_DIR / 'exploded_interface_diagnostic.png').relative_to(ROOT)),
        },
    }
    (MODEL_DIR / 'model_manifest.json').write_text(json.dumps(manifest, indent=2) + '\n')
    note = f"# {ASSET_ID}\n\n"
    note += ('diagnostic candidate' if candidate else 'NOT a candidate') + '. Scratch diagnostic only; not production/cloud accepted.\n\n'
    note += '## Construction Strategy\n\nCluster ownership, edge-chain measurements, and primary plane locking are preserved from v07. The changed experiment is assembly authoring: measured region boundaries produce canonical shared-interface reports, while the visible armor is one continuous closed skin from the measured exterior chain instead of seven visible prisms.\n\n'
    note += '## Gates\n\n'
    ...
    note += f"- Shape: {shape_status} IoU={depth_report['silhouette_iou']} p95={depth_report['p95_abs_depth_error']}\n- Silhouette edge gate: {edge_gate['status']} failures={edge_gate['failure_reasons']}\n- Shared interface gate: {shared_report['status']} failures={shared_report.get('failure_reasons', [])}\n- Assembly interface gate: {assembly_report['status']} failures={assembly_report.get('failure_reasons', [])}\n"
    note += f"- Candidate: {candidate}\n\n"
    note += '## Stats\n\n'
    note += f"- Vertices: {stats['vertices']}\n- Polygons: {stats['polygons']}\n- Triangles: {stats['triangles']}\n\n"
    note += '## Lesson\n\nThis pass is useful if it proves v07 edge-chain silhouette gains can survive a continuous shared-interface visible skin while authored pieces remain manifold. It is still red unless visual/cloud review later accepts the manufactured Sherman read.\n'
...
def main():
    ...
    shared_interfaces, shared_report = build_shared_interfaces(features)
    objects = []
    primitive_rows = []
    primary_skin, primary_skin_report = make_primary_visible_skin(features, mats['main_glacis'])
    objects.append(primary_skin)
    primitive_rows.append({
        'region': 'primary_visible_skin',
        'topology': 'single continuous closed armor skin',
        'source_use': 'measured source exterior edge chain plus v07 feature/interface measurements; no visible independent feature prisms',
        'source_sample_count': features['source_vertex_count'],
        'thickness': V08_SKIN_THICKNESS,
        'boundary_point_count': primary_skin_report['final_point_count'],
    })
    ...
    primitive_rows.extend([
        {'region': 'socket', 'topology': 'closed secondary cylinder', 'source_use': 'measured socket center/radius only; provisional surface diagnostic in v08 continuous-skin path', 'source_sample_count': features['features']['socket']['source_sample_count'], 'radius': features['features']['socket']['radius']},
        {'region': 'ring', 'topology': 'closed secondary annular prism', 'source_use': 'measured ring center/radius only; seated on continuous primary visible skin', 'source_sample_count': features['features']['ring']['source_sample_count'], 'inner_radius': features['features']['ring']['inner_radius'], 'outer_radius': features['features']['ring']['outer_radius']},
    ])
    ...
    assembly_report = assembly_interface_report(objects, shared_interfaces, primary_skin_report, depth_report)
    render_exploded_interface_diagnostic(features, shared_report, primary_skin_report)
    ...
    manifest = write_reports(features, objects, topology_reports, topology_gate, depth_report, area_report, primitive_rows, shared_report, assembly_report)
    print(json.dumps({
        'asset_id': ASSET_ID,
